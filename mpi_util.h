...
template<class T> size_t mpi_size(const Tensor<T> &m_obj);
template<class T> unsigned char* mpi_pack(unsigned char* m_ptr, const Tensor<T> &m_obj);
template<class T> unsigned char* mpi_unpack(unsigned char* m_ptr, Tensor<T> &m_obj);
...
/////////////////////////////////////////////////////////////////////////////////////////
template<class T>
size_t mpi_size(const Tensor<T> &m_obj)
{
	size_t len = mpi_size( m_obj.dim_size() );
	
	for(typename Tensor<T>::const_iterator i = m_obj.begin();i != m_obj.end();++i){
		len += mpi_size(*i);
	}
...
template<class T>
unsigned char* mpi_pack(unsigned char* m_ptr, const Tensor<T> &m_obj)
{
	// Pack the tensor length
	m_ptr = mpi_pack( m_ptr, m_obj.dim_size() );

	// Pack the tensor elements
	for(typename Tensor<T>::const_iterator i = m_obj.begin();i != m_obj.end();++i){
		m_ptr = mpi_pack(m_ptr, *i);
	}
...
template<class T>
unsigned char* mpi_unpack(unsigned char* m_ptr, Tensor<T> &m_obj)
{
	std::vector<typename Tensor<T>::size_type> len;

	m_ptr = mpi_unpack(m_ptr, len);
	
	m_obj = Tensor<T>(len);

	for(typename Tensor<T>::iterator i = m_obj.begin();i != m_obj.end();++i){
		m_ptr = mpi_unpack(m_ptr, *i);
	}
